...
Ink on paper, and one hairline rule. No accent colour is introduced, because nothing on the page
needs one to be understood. Colour arrives only inside the application captures, which are framed
consistently so their own palettes stay contained.

## Masthead

`assets/wordmark-light.svg` and `assets/wordmark-dark.svg` are the same 1000x300 composition with the
...
because the sentence is the point. Everything else is the space around them.
...
image so the image itself stays large. A hairline rule and a provenance line close each one, matching
the rule that opens it.
